...
layui.define('layer', function(exports){
    ...
    var submit = function(){
        ...
        if(field.class_type==1 && field.student_id.length>1){
            layer.msg("一对一只能添加一名学员", {
                icon: 5
                ,shift: 6
            });
            return false;
        ...
        $.post(url, field, function (data) {
            if (data.code == 1) {
                layer.msg(data.msg,{icon: 1,shade: 0.3, time: 2000},function () {
    ...
    //表单提交事件
    dom.on('submit(question_submit)', ELEM, submit)
